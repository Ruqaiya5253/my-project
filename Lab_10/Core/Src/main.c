...
void task1(void){
  ...
  myPrintf("Malloc Memory reserverd: %p \r\n", arr_malloc);
  if (arr_malloc == NULL) {
    return;
  }
  ...
  myPrintf("Calloc Memory reserved: %p \r\n", arr_calloc);
  if (arr_calloc == NULL) {
    // Error handleing
    free(arr_malloc);
    return;
  ...
  // Free everything so no memory leaks;
  myPrintf("Starting to free memory \r\n");
  free(arr_malloc);
  arr_malloc = NULL;
  free(arr_calloc);
  arr_calloc = NULL;
  myPrintf("Memory free! \r\n");
